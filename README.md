...
**The agent:** an EU AI Act compliance reviewer embedded in the developer's harness (OpenCode). It reviews AI product work at the moment it's written — including the coding agent's own output. When the agent writes to a product file or PRD, a plugin intercepts the write *before it lands* and runs a millisecond deterministic scan. A violation refuses the write with an article citation; the agent then reads the report, restructures to a compliant design (e.g. automatic account suspension → human review queue + human decision, the Art. 14 oversight pattern), and the rewrite passes the gate. Ambient KB-first reviews fire on file edits and session idle; a `compliance_review` tool gives on-demand verdicts; a CI workflow scans every PR; a global install brings the same gate to blank, unrelated workspaces.
